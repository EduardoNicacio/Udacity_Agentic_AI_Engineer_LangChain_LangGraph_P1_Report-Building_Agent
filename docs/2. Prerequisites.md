...
If you're working outside the Udacity workspace (such as your own local machine), use `pip install -r requirements.txt` to ensure required libraries are installed.
...
1. Finish implementing the function so that it supports ALL the `intent_type` parameters which could be "qa", "summarization", or "calculation"
